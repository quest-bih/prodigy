...
ct_all <- mutate(ct_all, has_publication = case_when(
  ...
  TRUE ~ NA  # Optional: catch-all for cases where neither condition is met (sets NA)
))
...
colnames(ct_all)
write.csv(ct_all, here::here("data", "clinical-trial-registry", "2-9-2024-analysis.csv"), row.names = FALSE)
